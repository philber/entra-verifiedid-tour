...
namespace AspNetCoreVerifiableCredentialsB2C
{
    ...
    public class ApiIssuerController : ApiBaseVCController
    {
        ...
        /// <summary>
        /// API to start an issuance request. If it contains the query string parameter 'id', 
        ...
        public ActionResult IssuanceReference() {
            ...
            try {
                ...
                if ( !string.IsNullOrWhiteSpace(correlationId) && cachedClaims != null) { // Added  && cachedClaims != null for the first call... so that the QR code is displayed
                ...
                // Get the manifest and check to see if there are any self asserted claims (id_token flow)
                JObject manifest = GetIssuanceManifest();
                Dictionary<string, string> claims = GetSelfAssertedClaims(manifest);
                if (claims.Count > 0 ) {
